...
public class Errer {
    ...
    public static String unickResult(Context context, String result) {
        if (result != null) {
            JSONObject resultJSON = JSON.parseObject(result);
            //状态
            boolean isStatus = resultJSON.getBoolean("Status");
...
//                    plantState.initToast(context, message, true, 0);
    ...
    //判断是不是合法JSON并解密
    public static String isResultLog(Context context, String result, RequestStatus status) {
        Log.e(TAG, "---Errer---" + result);
        if (status == RequestStatus.SUCCESS) {
            if (result != null) {
                JSONObject resultJSON = JSON.parseObject(result);
...
//                    plantState.initToast(context, message, true, 0);
     ...
     * @return
     */
    public static String evaluations(Context context, String result) {
        Log.e(TAG, "---Errer---" + result);
        if (result != null) {
            JSONObject resultJSON = JSON.parseObject(result);
            //状态
